import axios from "axios";
import { useEffect, useState } from "react";
import { addToCart, removeFromCart } from "../utils/cart";
import { FaTrash } from "react-icons/fa";

export default function BookingItem({ vehicleKey, qty, refresh }) {
    //state variables
    const [vehicle, setVehicle] = useState(null);
    const [status, setStatus] = useState("loading");

    //fetch vehicle data
    useEffect(() => {
        if (status === "loading") {
            axios
                .get(`${import.meta.env.VITE_BACKEND_URL}/api/products/${vehicleKey}`)
                .then((res) => {
                    setVehicle(res.data);
                    setStatus("success");
                })
                .catch((err) => {
                    console.error(err);
                    setStatus("error");
                    removeFromCart(vehicleKey);
                    refresh();
                });
        }
    }, [status]);

    if (status === "loading") {
        return <div className="text-accent">Loading...</div>;
    }

    if (status === "error") {
        return <div className="text-red-500">Failed to load product.</div>;
    }

    return (
        <div className="flex w-[600px] my-2 items-center gap-4 p-4 bg-primary shadow-md rounded-lg border border-secondary relative">
            {/* Remove item button */}
            <div className="absolute right-[-45px] text-red-500 hover:text-white hover:bg-red-500 p-[10px] rounded-full cursor-pointer">
                <FaTrash onClick={() => {
                    removeFromCart(vehicleKey);
                    refresh();
                }} />
            </div>

            {/* Product Image */}
            <img
                src={vehicle.image[0]}
                alt={vehicle.model}
                className="w-20 h-20 object-cover rounded-lg border border-secondary"
            />

            {/* Product Details */}
            <div className="flex flex-row items-center relative w-full">
                <h3 className="text-lg font-semibold text-accent">{vehicle.model}</h3>
                <div className="flex absolute right-0 gap-4">
                    <p className="font-medium w-[70px] text-center">
                        {typeof vehicle.dailyRate === 'number' ? vehicle.dailyRate.toFixed(2) : '0.00'}
                    </p>
                    <p className="font-medium w-[40px] text-center">
                        {qty}
                    </p>
                    <p className="text-lg font-semibold text-accent">
                        {(vehicle.dailyRate * qty).toFixed(2)}
                    </p>
                </div>
            </div>
        </div>
    );
}